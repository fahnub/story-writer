...
def finetune_completion(prompt, temp=0.7, top_p=1.0, tokens=300, freq_pen=0.0, pres_pen=0.0):
    max_retry = 20
    retry = 0
    while True:
        try:
            response = openai.Completion.create(
                model='davinci:ft-readrly:story-generation-2022-09-10-13-17-40',
                prompt=prompt,
                temperature=temp,
                max_tokens=tokens,
                top_p=top_p,
...
def gpt3_completion(prompt, engine='text-davinci-002', temp=0.7, top_p=1.0, tokens=300, freq_pen=0.0, pres_pen=0.0):
    max_retry = 5
    retry = 0
    while True:
        try:
...
if __name__ == '__main__':
    ...
    print(last_chunk)
    save_file(last_chunk, 'story/chunk0001.txt')
    ...
    for i in range(2, 10):
        prompt = open_file('prompt_full.txt').replace('<<OUTLINE>>', outline).replace('<<SUMMARY>>', summary_so_far).\
            replace('<<CHUNK>>', last_chunk)
        next_chunk = finetune_completion(prompt)
        print(next_chunk)
        save_file(next_chunk, 'story/' + make_filename(i))
        # update the summary
